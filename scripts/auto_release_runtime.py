...
AFTER that workflow has re-run the release-blocking gates (``unit-tests``,
``responsiveness-e2e``, and ``schema-sync``) inline and they are green (Gitea has
no ``workflow_run`` trigger, so the release workflow cannot listen on the ``ci``
workflow's success — it re-runs the gate itself).
...
entirely and only CUT THE TAG:
  ...
  2. Create the tag ``runtime-v<next>`` pointing at the exact event commit that
     passed all release gates, supplied by the workflow as ``github.sha``. Never
     re-read a moving ``main`` after the gates. Tag creation uses the Gitea tags API
     (``POST /repos/{owner}/{repo}/tags``) and targets the TAG ref, not the branch
     ref, so branch *push* protection does not block it (empirically proven: the
     manual ``runtime-v0.3.14`` cut returned HTTP 201 with this release/devops token).
...
def main(argv):
    ...
    p.add_argument(
        "--commit-sha",
        required=True,
        help="Exact 40-character github.sha that passed every release gate.",
    )
    p.add_argument(
        "--pyproject",
        type=Path,
    ...
    if not COMMIT_SHA_RE.fullmatch(args.commit_sha):
        print(
            "::error::--commit-sha must be an exact lowercase 40-character commit SHA",
            file=sys.stderr,
        )
        return 2
    ...
    tested_sha = args.commit_sha
    print(f"release-gated commit: {tested_sha[:9]}")

    if args.dry_run:
        print(f"DRY-RUN: would create tag {next_tag} at gated commit {tested_sha[:9]}")
        return 0

    create_tag(base, token, tag=next_tag, commit_sha=tested_sha, target=target)
    print(
        f"::notice::created tag {next_tag} at gated commit {tested_sha[:9]} "
        "— publish-runtime will fire"
    )
    return 0
